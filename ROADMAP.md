...
1. 🔴 **decide-first — automatic session transition.** The monitor tells the session it *should*
   hand off; nothing yet makes the hand-off cheap or automatic. Open: can an agent open a new
   session itself, or is `aiwbot` (which already drives swappable coding agents) the natural home?
   Investigate `claude --resume` / headless `-p` / the SDK before designing.
