...
>
> **A smaller WOS is the deliverable, not a side effect** (Lucas, 2026-08-24). Every session should
> aim to leave the workspace with **fewer lines than it found**, and a session that adds a feature
> owes the reduction that pays for it. This is a target on the total, not permission to write worse
> files: cut what repeats, what nobody reads, and what a generator can derive — never summarize a
> rule until it stops being enforceable. Where the two pull against each other, say so and let Lucas
> rule rather than trimming a law to hit a number.
