...
<!-- entropy:start -->
## B6 — ZCode reads the workspace hook registration but never runs it: a trust gate nobody documented

**Symptom:** found 2026-08-21, measured by probe ([`core/experiments/zcode-hook-protocol.md`](core/experiments/zcode-hook-protocol.md)). Every event in every fresh ZCode session logs `config.project_hooks.pending_trust` — *"Project hooks are pending workspace trust and remain blocked"* — so `.zcode/config.json` (the full edit-time enforcement registration, mirroring `.claude/settings.json`) is parsed and inert. ZCode sessions in this workspace run with zero gates until the trust is accepted in the client UI, once per machine.

**Why it is worse than a missing shim:** the registration exists and is versioned, the schema validates, and `test_shim_paths.py` resolves every spawn — every static check a shim owes is green while behaviourally nothing fires. That is exactly the ✅-without-firing shape `SPECS-shim.md` § *A ✅ in that table is a claim* was written against.

**Fix path:** accept workspace trust for `/mnt/workspace` in the ZCode client (an `agent: no` step in `SETUP.md`), then re-run the probe battery in a fresh session. The same run answers the still-open protocol questions (stdin shape, plain-text exit-2 fidelity, `${CLAUDE_PROJECT_DIR}`), which decide whether the direct registration stands or an adapter `core/hooks/zcode/zcode-hook.py` replaces it.

## B7 — ZCode has no memory path to symlink into `brain/memory`

**Symptom:** `memoryEnabled: true` in `~/.zcode/v2/setting.json`, but no memory directory exists anywhere under `~/.zcode` (`workspace/default` is empty, no `memory` key in any config or db table). The provider-agnostic storage norm (`AGENTS.md`) has nothing to anchor for ZCode the way `~/.claude/projects/<slug>/memory` anchors for Claude Code.

**Root cause:** unknown — whether ZCode persists memory at all on this build (3.8.1), and where, is undocumented. Revisit after the trust acceptance (B6) puts the runtime through real use; if a path appears, symlink it to `brain/memory` and close this.
...
2026-08-20 · 2396 tracked files scanned · **829 findings**, 265 of them here
...
| [`code/isoroll-content`](code/isoroll-content/ISSUES.md) | 100 |
| [`code/isoroll-module`](code/isoroll-module/ISSUES.md) | 120 |
