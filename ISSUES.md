...
## B8 — every Google OAuth token on this machine is world-readable
...
2026-08-28 · 2380 tracked files scanned · **37 findings here** (2026-08-25: 33 · +4 over 3 days) · 561 more across 27 nested repos, each in its own ISSUES.md
...
- ROADMAP.md — 213 lines, over the 200 cap; introduced by feeca22 lsfcin
- SETUP.md — 488 lines, over the 200 cap; introduced by 3e575bb lsfcin
...
- brain/INBOX.md — 1 line(s) over the 120-column cap (first at line 28)
...
- brain/goals/home-casinhas.md — 5 line(s) over the 120-column cap (first at line 60)
...
- core/SPECS.md — 267 lines, over the 200 cap; introduced by 4c3d952 lsfcin
- core/hooks/SPECS.md — 331 lines, over the 200 cap; introduced by 0237832 lsfcin
...
- core/tools/wos — 9 code files in one directory, over the WARN_FILES signal; split by responsibility if the split removes more table than the hop adds
...
2026-08-27 · `make verify-full` · **red**
...
E       AssertionError: AGENTS.md and core/norms/ disagree — run core/hooks/routing/norms.py
E       assert ['FILESYSTEM ...tering.', ...] == ['FILESYSTEM ...tering.', ...]
E         
E         At index 1 diff: "**PROVIDER-AGNOSTIC STORAGE: the workspace owns its state, never a harness.** Agnostic to provider, company and harness — so nothing that matters is written to a vendor's private directory. If a harness insists on its own path, symlink that path into the repo. Live case: `~/.claude/projects/<slug>/memory` → `brain/memory/`, so what the agent writes lands in git." != '**PROVIDER-AGNOSTIC STORAGE**: the workspace owns its state, never a harness, if it insists, symlink that path into WIS.'
E         Left contains one more item: '**AGENT-FACING TEXT NAMES ONE ACTION....
E         
E         ...Full output truncated (2 lines hidden), use '-vv' to show

core/tools/test/wos/test_norms.py:69: AssertionError
=========================== short test summary info ============================
FAILED core/tools/test/wos/test_norms.py::test_every_norm_file_is_declared_and_every_declared_norm_exists
FAILED core/tools/test/wos/test_norms.py::test_the_always_loaded_block_matches_its_sources
2 failed, 570 passed in 60.35s (0:01:00)
make: *** [Makefile:25: verify-full] Error 1
```
<!-- verify:end -->
