...
three times. Five separate definitions of "a code file" existed across different checkers before
they were unified behind `file_law.is_code_file`, guarded now by
...
**The third (2026-08-24) is the one worth recognising by shape, because it never looked like
drift.** `core/tools/wos/wrap` carried a private exemption for YAML frontmatter, and said so in its
own docstring — *"this adds the two shapes that are not its business"*. It was honest, documented,
and still wrong: the law kept flagging those lines while the tool refused to touch them, so the
count charged for a rule nothing intended to enforce, and the disagreement was invisible because
each file read correctly on its own. **A tool that knows it is overriding the law has found a gap
in the law, not a special case of its own** — the fix is to move the rule up, not to comment the
override. The tell is a checker whose docstring explains why it disagrees with what it just asked.
...
| [`SPECS-shim.md`](SPECS-shim.md) | One canonical behaviour, and what a new agent runtime must do to get it. | core/hooks/copilot/, core/hooks/zcode/, core/hooks/hook_input.py, .opencode/plugins/, .zcode/config.json |
<!-- routing:end -->
