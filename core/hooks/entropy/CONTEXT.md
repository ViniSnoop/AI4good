...
| [`entropy_context.py`](entropy_context.py) | [`entropy_context.pyi`](entropy_context.pyi) | `check_inventory`, `context_head`, `check_misplaced_answer`, `is_project`, `check_goal_link` | Tier 0 CONTEXT.md rules (ROADMAP.md Frente 4.1). Zero-token, deterministic. |
| [`entropy_corpus.py`](entropy_corpus.py) | [`entropy_corpus.pyi`](entropy_corpus.pyi) | `tracked_files`, `nested_repos`, `is_generated_mirror`, `enforcement_paths`, `wiki_exempt_paths` | Which files the Tier 0 checks look at, and which of them are allowed to name what the |
...
| [`entropy_ledger.py`](entropy_ledger.py) | [`entropy_ledger.pyi`](entropy_ledger.pyi) | `retired_hits`, `item_slugs`, `duplicate_slugs`, `finished_work_hits`, `unanswered_placeholders` | Tier 0 ledger and vocabulary checks (ROADMAP.md Frente 4.1). Zero-token, deterministic. |
