...
if [ -x /mnt/workspace/core/hooks/gitflow-gate.sh ]; then
  /mnt/workspace/core/hooks/gitflow-gate.sh || exit 1
fi
...
python3 /mnt/workspace/core/hooks/type-gate.py || exit 1

# ── 1f. Nested-gitlink gate (workspace repo) ──────────────────────────────────
if [ -x /mnt/workspace/core/hooks/nested-gitlink-gate.sh ]; then
  /mnt/workspace/core/hooks/nested-gitlink-gate.sh || exit 1
fi
