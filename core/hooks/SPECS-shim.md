...
> governs: core/hooks/copilot/, core/hooks/zcode/, core/hooks/hook_input.py, .opencode/plugins/, .zcode/config.json
...
*Registered but measured **inert** until workspace trust is granted in the ZCode client — ISSUES.md B6, experiment [`core/experiments/zcode-hook-protocol.md`](../experiments/zcode-hook-protocol.md). No zcode cell below reads ✅ until a post-trust probe run earns it.
...
**⏸ means registered, not firing.** `.zcode/config.json` names every canonical script the
Claude Code column names, but a measured trust gate holds them inert (ISSUES.md B6) — so the
honest cell is not ✅ but ⏸, and it flips only when a post-trust probe run verifies each row.
Event remaps: no `SubagentStart` in zcode, so `agent-context.py` rides PreToolUse `Agent|Task`;
no `PreCompact`, so `precompact-wipe.sh` rides SessionStart `^compact$` (post-compaction —
same wipe, later moment).
