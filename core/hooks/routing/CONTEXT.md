...
| [`shard_table.py`](shard_table.py) | [`shard_table.pyi`](shard_table.pyi) | `shard_facts`, `render_table`, `shards_of`, `index_for`, `build_shard_rows` | A sharded type's index table: what each TYPE-<slug>.md publishes, rendered so the index answers |
