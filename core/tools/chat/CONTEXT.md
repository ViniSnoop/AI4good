...
```
core/tools/chat/wazip <export.zip>... --out <dir> [--prompt-file <carrier.txt>] [--keep-media <dir>]
```
...
|------|-----|-------------|
| [`chat_stitch.py`](chat_stitch.py) | `transcript_for`, `fold`, `is_noise`, `redact_line`, `redact` | chat_stitch.py — a chat export becomes one readable conversation: every "audio attached" line gains what was actually said underneath, bot menus that repeat verbatim go, and secrets are redacted. |
| [`chat_transcribe.py`](chat_transcribe.py) | `duration`, `audios`, `sidecar`, `eta`, `run` | chat_transcribe.py — batch speech-to-text over an extracted chat export; one .txt sidecar per audio. Resumable: an audio whose sidecar already exists is skipped, so a killed run loses nothing. |
