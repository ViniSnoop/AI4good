...
| File | Description |
|------|-------------|
| [`test_chat_stitch.py`](test_chat_stitch.py) | T1 chat stitching: an audio line must never lose what was said, a bot menu must never survive, and a secret must never reach a versioned file. Zero-token, no network, no model. |
<!-- routing:end -->
