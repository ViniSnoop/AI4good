...
MARKER = WORKSPACE_ROOT / 'core/hooks/git/branch-marker.sh'
...
def marker_path(repo: Path) -> Path:
	...
	return Path(f'/tmp/claude_branch_{sanitized}.txt')
...
def run(repo: Path, mode: str):
	done = subprocess.run(['bash', str(MARKER), mode], cwd=repo,
...
def test_outside_a_repo_it_says_nothing(tmp_path):
	done = subprocess.run(['bash', str(MARKER), 'check'], cwd=tmp_path,
	                      capture_output=True, text=True)
