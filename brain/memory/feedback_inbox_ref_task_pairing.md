...
When `/inbox` routes a reference/link that carries actionable intent (Lucas's note says "investigar", "útil pro X", "pode servir pra Y", or the content plainly asks to evaluate/adopt), it must create BOTH a `refs/REFS.md` line AND a paired assessment task in the owning surface (project `ROADMAP.md ## Backlog`, or the backlog of the goal it serves in `brain/goals/*.md` for cross-cutting/life — `brain/INBOX.md` if no goal owns it yet). Pure archival refs with no intent stay ref-only.
